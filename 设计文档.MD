...
* **功能**：从指定 Excel 文件中读取“产品名称”列，随机选择一个产品。
...
* **接入方式**：通过阿里云百炼智能体 MCP 服务进行联网搜索，替换原生 HTTP API 调用
* **阿里智能体应用配置**：
 ![ali联网搜索智能体.png](ali%E8%81%94%E7%BD%91%E6%90%9C%E7%B4%A2%E6%99%BA%E8%83%BD%E4%BD%93.png)
* **主要方法**：
...
* **功能**：基于业务目标与预算，调用 OpenAI/GPT 模型生成关键动作序列和时间节点。
* **主要方法**：
...
* **主要方法**：
  ...
  * `run(context: dict) -> dict`
    ...
    * Prompt 包含 `product`、`audience`、`tone` 等信息
...
* **环境依赖**：
  ...
  * Python 3.8+
  * pandas, requests, openai, dashscope
* **环境变量**：
